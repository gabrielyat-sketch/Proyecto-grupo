...
describe('ClientePacientes', () => {
  ...
  it('devuelve el paciente cuando el servicio responde bien', async () => {
    ...
    await expect(new ClientePacientes(env).obtener('p-1', 'Bearer x')).resolves.toMatchObject({
      id: 'p-1',
    });
  });
  ...
  it('un 403 se traduce a falta de permiso', async () => {
    ...
    await expect(new ClientePacientes(env).obtener('p-1', 'Bearer x')).rejects.toThrow(/permiso/i);
  });
